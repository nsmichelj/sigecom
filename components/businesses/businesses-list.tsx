...
export function BusinessesList() {
  ...
  const {
    ...
    queryKey: ["businesses"],
    queryFn: async () => {
      const { data: businesses, success } = await getBusinessesAction();
      if (!success || !businesses) {
  ...
  if (isError) {
    return (
      <div className="p-8 text-center text-muted-foreground bg-muted/50 rounded-lg border border-dashed">
        Hubo un error al cargar los emprendimientos.
      </div>
    );
  }
    ...
    return (
      <div className="p-12 text-center bg-muted/50 rounded-lg border border-dashed flex flex-col items-center gap-3">
        ...
        <div className="space-y-1">
          <h3 className="font-medium">No hay emprendimientos</h3>
          <p className="text-sm text-muted-foreground">
            Aún no has registrado ningún emprendimiento o negocio comunitario.
          </p>
        </div>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                      <Link href={`/dashboard/businesses/${item.id}`}>
                        <Eye />
                        Ver detalle
                      </Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild>
                      <Link href={`/dashboard/businesses/${item.id}/edit`}>
                        <Edit />
                        Editar
                      </Link>
                    </DropdownMenuItem>
